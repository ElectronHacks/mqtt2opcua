 ...
 Forward and backward handlers - see below for example
    ...
    // function post_initialize() {
	    ...
	    // var endpointUrl = server.endpoints[0].endpointDescriptions()[0].endpointUrl;
            ...
            // if (!(server.mqtt = mqtt.connect(mqttURL,{ username:'guest', password:'12345'}))) {
        ...
        // if (options.hasOwnProperty('cache') ) {
            ...
            // var exitHandler = function (code) {
               // try {
                   // var fd;
                   // if (!(fd = fs.openSync(options.cache, "w+"))){
                       // console.error(err);
                   // } else {
                       // fs.writeSync(fd,JSON.stringify(persist));
                       // fs.closeSync(fd);
                       // console.log(options.cache + " written");
                       // code = 0;
                   // }
               // } catch (e) {
                   // console.error("Failed to write cache " + e.toString());
               // } finally {
                  // process.exit(code);
               // }
           // }
            ...
            // try{
                ...
                // fs.statSync(options.cache).isFile();
                // persist = JSON.parse(fs.readFileSync(options.cache));
                // for (var topic in persist){
                    // if (persist[topic] != undefined){
                        // onMessage(topic,String(persist[topic]));
                    // }
               // }
            ...
            // } catch (e) {
                // console.error(e);
                // if (e.code != "ENOENT"){
                    // console.error("Unable to load cache " + (e.code !== undefined  ? e.code : "check file for corruption " + options.cache))
                    // process.exit(1);
                // } else {
                    // fs.open(options.cache, "w+", function(err,fd){
                        // if (err != undefined){
                            // console.error("Unable to create cache: " + err);
                            // process.exit(1);
                        // }
                        // fs.writeSync(fd,"{}\n",0);
                        // fs.closeSync(fd);
